...
Flesh out the Discovery.files() method to scan the source directories, filter files according to the configuration (e.g., extensions, blacklist), create a list of ContentItem and AssetItem objects.

2. Complete the Configuration loading and validation:

Implement the logic in Config.load() and Config.validate() to properly parse the config.toml file, populate the Config object with all the nested settings (like site, build, permalinks), validate that all required paths and settings are present and correctly formatted.

3. Implement the backend for the Cache service:
    
    - Set up the SQLite database
    - create the build_items and content_metadata tables.
    - Implement Cache class.

*Summary**

The core is a build pipeline processing content into a static website,
with a plugin-style service architecture.
Here's a file-by-file breakdown:

- `pgen/main.py`: The application's entry point, defining build and clean commands via click and managing command-line arguments.
- `pgen/__init__.py`: An empty file marking the pgen directory as a Python package.
- `pgen/buildpipeline.py`: Defines the build pipeline's data structures (BuildItem, ContentItem, AssetItem) and 
    a BuildState enum (RAW, VALIDATED, WRITTEN) to track item processing.
- `pgen/config.py`: Manages site configuration through a dataclass, loading settings from config.toml.
- `pgen/site.py`: The main class that orchestrates site generation, initializing the configuration, services, and pipeline. 
    It manages services like caching and templating via a ServiceRegistry.
- `pgen/validation.py`: Contains a placeholder Validation class, intended for data and configuration validation.
- `pgen/stages/discovery.py`: Features a placeholder Discovery class meant to find and classify content and asset files.
- `pgen/services/cache.py`: A placeholder Cache class for managing build artifacts.
- `pgen/services/files.py`: A placeholder Files class for file-related operations.
- `pgen/services/metadata.py`: A placeholder Metadata class, likely for handling content metadata.
- `pgen/services/output.py`: Defines an Output class with a placeholder clean method for managing the build output directory.
- `pgen/services/permalink.py`: A placeholder Permalink class for generating content permalinks.
- `pgen/services/registry.py`: Provides a ServiceRegistry for central management of all services, 
    supporting registration, initialization, and access.
- `pgen/services/reporter.py`: Defines a Reporter class for collecting and reporting build issues with varying severity levels.
- `pgen/services/template.py`: A placeholder Template class designed to handle different templating engines.
- `tests/test_buildpipeline.py`: Contains pytest unit tests for the BuildItem class, verifying its state machine and transition rules.
